...
public class ShareBoxUtil {
	...
	public static Response getAllFiles() throws Exception {
		rSpec = RestAssured.given().relaxedHTTPSValidation();
		response = rSpec.queryParam("token", token1).when().get(filesParam);
		if (response.getStatusCode() != 200) {
			throw new Exception(
	...
	public static Response getAllFiles(String getSharedFiles) throws Exception {
		rSpec = RestAssured.given().relaxedHTTPSValidation();
		response = rSpec.queryParam("token", token1).queryParams("getSharedFiles", getSharedFiles).when()
				.get(filesParam);
		if (response.getStatusCode() != 200) {
			throw new Exception(
					"Failed to get all file details : " + response.getStatusCode() + " " + response.getStatusLine());
...
//			throw new Exception("Failed to share file : " + response.getStatusCode() + " " + response.getStatusLine());
	...
	public static Response acceptOrRejectFile(String fileId, boolean isAccepted) throws Exception {
		rSpec = RestAssured.given().relaxedHTTPSValidation();
		response = rSpec.queryParam("token", token1).queryParams("fileId", fileId).queryParam("isAccepted", isAccepted)
				.when().put(filesParam);
